...
date: 2017-10-17T00:29:41+09:00
lastmod: 2017-10-25T20:42:10+09:00
---
...
* **回帰分析 (regression analysis)**
    * 2 変数$X, Y$のデータがあるとき**回帰方程式 (regression equation)**を求めること
...
#### 13.3.1 偏回帰係数の標本分布
* 誤差項が 3 つの条件に加えて正規分布$N(0, \sigma^2)$に従うとする
* $\beta_2$は$N(\beta_2, \frac{\sigma^2}{\sum(X_i-\bar{X})^2}$に従う
    * $\sigma^2$は未知なので標準誤差を用いる
    * $s.e.(\hat{\beta_2})=\frac{s.e.}{\sqrt{\sum(X_i-\bar{X})}}$
    * 標準化した$\hat{\beta_2}$の$\sigma$を$s.e.$で置き換えた$t_2=\frac{\hat{\beta_2}-\beta_2}{s.e.(\hat{\beta_2})}$は自由度$n-2$の t 分布に従う
